...
public class Easthaven extends Applet implements MouseListener, MouseMotionListener
{
	...
	if (e.getButton () == 1)
	{
		...
		if (tableau [i].isPointInsideFaceUpCards (e.getX (), e.getY ()))
		{
		    ...
		    tempDeckLength = tableau [i].getDeckLength ();
		    // Add cards to dragging card class and check if in order
		    for (int j = tableau [i].cardTabClicked (e.getX (), e.getY ()) ; j < tempDeckLength ; j++)
		    {
		...
		if (dealDeck.getDeckLength () >= tableau.length)
		{
			...
			tableau [i].addCard ((CardClass) dealDeck.getTopCard ());
			dealDeck.removeTopCard ();
		    }
		}
		else
	...
	if (draggingCard.getDeckLength () == 1)
	{
	    ...
	    for (int i = 0 ; i < foundations.length ; i++)
	    {
		// If mouse is inside the foundation, a card is being dragged, the card suit
		// fits, and it is in ascending order
		// or foundation is empty
		if (foundations [i].isPointInside (e.getX (), e.getY ()) && draggingCard.getDeckLength () == 1
			&& ((foundations [i].getDeckLength () == 0
				&& ((CardClass) draggingCard.getTopCard ()).getValue () == 1)
			    || foundations [i].getDeckLength () > 0 && ((CardClass) foundations [i].getTopCard ())
			    .getValue () == ((CardClass) draggingCard.getTopCard ()).getValue () - 1)
			&& foundations [i].getSuit () == ((CardClass) draggingCard.getTopCard ()).getSuit ())
		{
	...
	else if (draggingCard.getDeckLength () > 1)
	{
	    tempDeckLength = draggingCard.getDeckLength ();
	    for (int i = 0 ; i < tableau.length ; i++)
	    {
		// If mouse is inside the piles, cards are being dragged, the pile would be in
		// descending order, and the pile is in alternating colours
		if (tableau [i].isPointInsideFaceUpCards (e.getX (), e.getY ())
			&& ((CardClass) tableau [i].getTopCard ())
			.getValue () == ((CardClass) draggingCard.getBottomCard ()).getValue () + 1
			&& (((CardClass) tableau [i].getTopCard ()).getSuit ()
			    + ((CardClass) draggingCard.getBottomCard ()).getSuit ()) % 2 == 1)
		{
		    for (int j = 0 ; j < tempDeckLength ; j++)
		    {
			tableau [i].addCard ((CardClass) draggingCard.getBottomCard ());
