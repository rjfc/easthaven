...
public class Easthaven extends Applet implements MouseListener, MouseMotionListener
{
	...
	// BufferGraphics variables
	dim = getSize ();
	// Prevents bugs during game restart
	if (mouseInitialized)
	{
	...
	restartGameButton.setCentre (getWidth () / 2, getHeight () / 2 - 70);
	restartGameButton.setWidth (150);
	restartGameButton.setHeight (60);
	...
	if (endGameButton.isPointInside (e.getX (), e.getY ()) || restartGameButton.isPointInside (e.getX (), e.getY ()))
	{
	...
	if (e.getButton () == 1)
	{
	    // Make sure the following code only works if game is active
	    if (!gameEnded)
	    {
		for (int i = 0 ; i < piles.length ; i++)
		{
		...
		for (int i = 0 ; i < foundations.length ; i++)
		{
		    // If user clicks on a foundation and it is not empty
		    if (foundations [i].isPointInside (e.getX (), e.getY ()) && foundations [i].getDeckLength () > 0)
		    {
	    ...
	    // If user clicks on restart game button
	    if (restartGameButton.isPointInside (e.getX (), e.getY ()) && (gameEnded || gameWon))
	    {
		init ();
	    }
	...
	if (draggingCard.getOriginalPile () > -1 && piles [draggingCard.getOriginalPile ()].getDeckLength () > 0)
	{
