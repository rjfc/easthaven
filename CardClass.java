...
public class CardClass extends ShapeClass {
	...
	// Different fonts
	private Font f1 = new Font("SanSerif", Font.BOLD, 14);
	private Font f2 = new Font("SanSerif", Font.BOLD, 18);
	private Font f3 = new Font("SanSerif", Font.BOLD, 22);
	private Font f4 = new Font("SanSerif", Font.BOLD, 24);
	...
	// If point is inside the top of a card 
	public boolean isPointInsideTab(int x, int y) {
		if (x >= iCentreX - iWidth / 2 && x <= iCentreX + iWidth / 2 && y >= iCentreY - iHeight / 2
				&& y <= iCentreY - iHeight / 2 + 25) {
			return true;
		}
		return false;
	}
	...
	public void draw(Graphics g) {
		...
		if (isFaceUp) {
			...
			case 0:
				...
				s1.draw(g);
				s1.setCenter(iCentreX + iWidth / 2 - 11, iCentreY - iHeight / 2 + 13);
				s1.setHeight((int) Math.round(iHeight * 0.2));
				s1.setWidth((int) (s1.getHeight() * 0.8));
				s1.setColor(Color.black);
				s1.draw(g);
				break;
			case 1:
				...
				d1.draw(g);
				d1.setCenter(iCentreX + iWidth / 2 - 11, iCentreY - iHeight / 2 + 13);
				d1.setHeight((int) Math.round(iHeight * 0.2));
				d1.setWidth((int) (d1.getHeight() * 0.8));
				d1.setColor(Color.red);
				d1.draw(g);
				break;
			case 2:
				...
				c1.draw(g);
				c1.setCenter(iCentreX + iWidth / 2 - 11, iCentreY - iHeight / 2 + 13);
				c1.setHeight((int) Math.round(iHeight * 0.2));
				c1.setWidth((int) (c1.getHeight() * 0.8));
				c1.setColor(Color.black);
				c1.draw(g);
				break;
			case 3:
				...
				h1.draw(g);
				h1.setCenter(iCentreX + iWidth / 2 - 11, iCentreY - iHeight / 2 + 13);
				h1.setHeight((int) Math.round(iHeight * 0.2));
				h1.setWidth((int) (h1.getHeight() * 0.8));
				h1.setColor(Color.red);
				h1.draw(g);
				break;
			...
			if (value > 1 && value < 11) {
				if (value == 10) {
					g.drawString(Integer.toString(value), iCentreX + (int) (iWidth * 0.1),
							iCentreY + (int) (iHeight * 0.45));
					g.drawString(Integer.toString(value), iCentreX - (int) (iWidth * 0.45),
							iCentreY - (int) (iHeight * 0.275));
				} else {
					g.drawString(Integer.toString(value), iCentreX + (int) (iWidth * 0.245),
							iCentreY + (int) (iHeight * 0.45));
					g.drawString(Integer.toString(value), iCentreX - (int) (iWidth * 0.45),
							iCentreY - (int) (iHeight * 0.275));
				}
